...
getActividadCargoOr(idpuesto){
  ...
  const url = URL_SERVICE.url + '/actividadCargo/ordenar/'+this.idEmp+'/'+idpuesto;
...
getActividadCargoxPuesto(idP: string){
  const token = localStorage.getItem('tokenFact');
  const headers = new HttpHeaders({
      'Content-Type': 'application/x-www-form-urlencoded',
      token
  ...
  const url = URL_SERVICE.url + '/actividadCargo/'+this.idEmp+ "/" + idP ;;
...
eliActCarg(id: string){
  ...
  const url = URL_SERVICE.url + '/actividadCargoEliminar/' + id;
